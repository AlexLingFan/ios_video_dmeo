...
public class AlexMediaCacher<Location: AlexMediaCacheLocation> {
    ...
    /// 正在进行的缓存
    public private(set) var currentCache: AlexMediaCacheManager?
    
    /// 暂停的缓存
    private var suspendedCaches: Set<AlexMediaCacheManager> = []
    
    public weak var delegate: AlexMediaCacherDelegate?
    ...
    public func makeCache(location: Location, cacheImmediately: Bool = false) -> AlexMediaCacheManager? {
...
//                return cache
        ...
        //创建新视频
        let cache: AlexMediaCacheManager
        ...
        if
            ...
            //创建一个新视频
            cache = AlexMediaCacheManager(cacheLocation: location, cacheListPath: cacheListPath, delegate: self)
        }
    ...
    public func isUrlCached(url: String) -> Bool {
        if cacheList.contains(url.Alexmd5) {
            return true
        }
        return false
    }
    ...
    /// 开始视频缓存
    public func startCache(_ cache: AlexMediaCacheManager) {
    ...
    /// 暂停视频缓存
    public func suspendCache(_ cache: AlexMediaCacheManager) {
    ...
    /// 停止视频缓存，并清除临时数据
    public func stopCache(_ cache: AlexMediaCacheManager) {
    ...
    /// 删除视频缓存（未启用）
    private func removeCache(_ cache: AlexMediaCacheManager) {
...
//MARK: - 直接对磁盘文件进行操作的方法
extension AlexMediaCacher {
    
    /// 删除视频缓存
    public func removeCache(located location: Location) {
        
        let cache = AlexMediaCacheManager(cacheLocation: location, cacheListPath: cacheListPath, delegate: self)
        removeCache(cache)
    }
}

extension AlexMediaCacher: AlexMediaCacheDelegate {
