...
public enum AlexMediaType {
    case audio
    case video
}

public class AlexMediaCacheManager: NSObject {
    
    public let identifier: String
    public let remoteURL: URL
    
    private weak var delegate: AlexMediaCacheDelegate?
    private let cacheLocation: AlexMediaCacheLocation
    ...
    public private(set) var asset: AVAsset? {
        ...
        didSet {
            ...
            if let asset = asset, asset is AVURLAsset {
                ...
                let outputFileExt = AlexMediaCacheManager.preferredLocalPathExtension(from: cacheLocation.mediaType)
                ...
                guard ext == outputFileExt else {
                    exportSession = nil
                    print("文件输出类型不支持，请检查AlexMediaCacheLocation中playURL的扩展名")
                    return
                }
    ...
    init(cacheLocation: AlexMediaCacheLocation, cacheListPath: String, delegate: AlexMediaCacheDelegate) {
    ...
    deinit {
        ...
        print("====================")
        print("AlexMediaCacheManager deinit")
        print("停止缓存任务")
        print("删除未加密的文件")
        print("====================")
    ...
    public class func preferredLocalPathExtension(from mediaType: AlexMediaType) -> String {
        switch mediaType {
        case .audio:
            return "caf"
        case .video:
...
extension AlexMediaCacheManager: AVAssetResourceLoaderDelegate {
    
}

extension AlexMediaCacheManager {
    ...
    func createDecryptedFile() -> URL? {
        ...
        do {
            ...
            try AlexMediaCryptor.endecryptFile(at: cacheLocation.playURL)
    ...
    private func createEncryptedFile() -> URL? {
        ...
        do {
            ...
            try AlexMediaCryptor.endecryptFile(at: cacheLocation.storageURL)
