...
class AlexAudiovisualCommonManager: NSObject {
    
    weak var playerView: AlexPlayerCommonView?
    
    /// 隐藏状态栏计时器
    var hideTimer: Timer?
    
    /// 当前音视频配置
    var playerConfig: AlexPlayerCommonConfig? {
        willSet {
            if let newConfig = newValue {
                ...
                if let oldConfig = playerConfig {
                    ...
                    if AlexReach.isReachableViaWWAN() {
                        print("当前使用手机流量")
    ...
    /// 当前播放器状态
    var playerStatus: AlexAudiovisualStatus? {
        didSet {
    ...
    /// 断点续播节点列表
    private var playContiuneListPath: String = AlexFileDirectory.PlayContinue.listPlist
    /// 当前播放地址
    ...
    convenience init(_ view: AlexPlayerCommonView) {
        self.init()
    ...
    /** 初始化播放器*/
    private func updatePlayerItem(config: AlexPlayerCommonConfig) {
    ...
    /** 播放视频*/
    private func playVideo(urlStr: String, config: AlexPlayerCommonConfig) {
        ...
        func playVideo(item: AVPlayerItem) {
            ...
            playerView?.noNetView.isHidden = AlexReach.isReachable()
            ...
            if config.playContinue {
                if let playContinueList = NSDictionary(contentsOfFile: playContiuneListPath) {
                    ...
                    if let playContinueTime = playContinueList[urlStr.Alexmd5] as? Double,
        ...
        if config.needCache && !AlexReach.isReachableViaWWAN() && isOnlineSource {
            // 缓存的处理
            let location = AlexDefaultVideoCacheLocation(remoteURL: playerUrl, mediaType: isVideo ? .video : .audio, authenticationFunc: config.authenticationFunc)
            let canCache = AlexCacheStorage.available > Float(500 * 1024 * 1024)
                ...
                if let asset = cache.asset {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if self.playerView?.videoCacher.cacheList.contains(cache.identifier) == true {
                            self.playerView?.controlPanel.cacheView.frame = CGRect(x: 48, y: 20, width: Alex_SCREEN_WIDTH - 176, height: 1)
                        }
                    }
    ...
    private func initViewWithConfig(config: AlexPlayerCommonConfig) {
    ...
    func getVideoFrame() -> CGRect {
        ...
        return CGRect(x: 0, y: 0, width: Alex_SCREEN_WIDTH, height: Alex_SCREEN_WIDTH / 16 * 9)
    }
    ...
    func setPlayContinue() {
            ...
            let currentTime = playerView?.videoPlayer?.currentItem?.currentTime().seconds
        {
            if let playContinueList = NSDictionary(contentsOfFile: playContiuneListPath),
                ...
                // 已存在记录 -> 添加
                playContinueDic[urlStr.Alexmd5] = currentTime
                (playContinueDic as NSDictionary).write(toFile: playContiuneListPath, atomically: true)
            } else {
                // 未存在 —> 新增
                ([urlStr.Alexmd5: currentTime] as NSDictionary).write(toFile: playContiuneListPath, atomically: true)
            }
        }
    }
}
...
//MARK: 控制面板相关
extension AlexAudiovisualCommonManager {
    /** 刷新播放器控制面板（进度｜时间）*/
