...
class AlexPlayerCommonView: UIView {
    
    /// 播放器回调
    weak var delegate: AlexPlayerCommonViewDelegate?
    /// 播放器的父view
    ...
    /// 控制播放面板
    var controlPanel: AlexControlPanelView!
    /// 全屏状态下遮罩
    var fullMaskView: AlexFullScreenMaskView?
    /// 音频播放界面（遮罩）
    var audioPlayView: AlexAudioPlayView?
    /// 播放完毕之后展示
    var endPlayView: AlexEndPlayView?
    /// 快捷操作提醒
    var speedyRemindView: AlexSpeedyRemindView?
    /// 无网提示界面
    lazy var noNetView: AlexWithoutNetView = {
        let view = AlexWithoutNetView()
        view.isHidden = true
        view.checkNetBtn.addTarget(self, action: #selector(checkLocalNet), for: .touchUpInside)
        return view
    }()
    /// 视频加速指示
    private lazy var accelerationView: AlexAccelerationView = {
        let view = AlexAccelerationView()
        view.isHidden = true
        view.alpha = 0
    ...
    /// 视频缓存
    let videoCacher: AlexMediaCacher = AlexMediaCacher<AlexDefaultVideoCacheLocation>()
    
    //MARK: Private Config
    /// 长按加速度
    private let playerAcceleration: Float = 2
    /// 普通事件管理
    private var manager: AlexAudiovisualCommonManager?
    /// 快捷操作管理
    private var speedyManager: AlexAudiovisualSpeedyManager?
    /// 播放进度计时器
    ...
    convenience init(_ baseView: UIView) {
        ...
        fatherView = baseView
        manager = AlexAudiovisualCommonManager(self)
        speedyManager = AlexAudiovisualSpeedyManager(self)
...
//        videoView.addGestureRecognizer(videoViewLongPress)
        ...
        audioPlayView = AlexAudioPlayView()
        ...
        speedyRemindView = AlexSpeedyRemindView()
    ...
    private func createControlPanel() {
        controlPanel = AlexControlPanelView()
    ...
    private func createAllScreenNavigationBar()  {
        fullMaskView = AlexFullScreenMaskView()
    ...
    deinit {
        ...
        print("AlexPlayerCommonView Deinit")
    }
}

//MARK: 公共配置方法
extension AlexPlayerCommonView {
    
    /// 更新当前播放内容
    /// - Parameter commonConfig: 播放配置
    func updateCurrentPlayer(playerConfig: AlexPlayerCommonConfig) {
...
extension AlexPlayerCommonView {
    ...
    private func dealForFullScreenPlayer(orientationChange: Bool = false)  {
        ...
        controlPanel?.screenButton.setImage(UIImage(named: "alex_video_ic_fullscreen", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        controlPanel.snp.remakeConstraints { (make) in
            make.leading.trailing.equalToSuperview()
            make.height.equalTo(40)
            make.bottom.equalTo(Alex_IS_IPHONEX ? -15 : 0)
        }
    ...
    private func dealForNormalScreenPlayer() {
        ...
        controlPanel?.screenButton.setImage(UIImage(named: "alex_video_ic_normalscreen", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        controlPanel.snp.remakeConstraints { (make) in
            make.bottom.leading.trailing.equalToSuperview()
            make.height.equalTo(40)
        }
        ...
        playerLayer?.frame = manager?.getVideoFrame() ?? CGRect(x: 0, y: 0, width: Alex_SCREEN_WIDTH, height: Alex_SCREEN_WIDTH / 16 * 9)
...
extension AlexPlayerCommonView {
    ...
    @objc func applicationDidBecomeActive() {
        ...
        noNetView.isHidden = AlexReach.isReachable()
    ...
    @objc private func playerViewLongPressed(_ sender: UILongPressGestureRecognizer) {
        if manager?.playerStatus == .playing {
            // 获取当前播放倍速
            let currentSpeed = UserDefaults.standard.value(forKey: "AlexPlayer_rate") as? Float ?? 1
...
//MARK: 更多功能调整
extension AlexPlayerCommonView: AlexFullScreenMaskViewDelegate {
    /** 更改播放器播放速度*/
...
//MARK: 缓存回调
extension AlexPlayerCommonView: AlexMediaCacherDelegate {
    /** 缓存进度更新*/
    func cacher<LocationType>(_ cacher: AlexMediaCacher<LocationType>, cacheProgress progress: Float, of cache: AlexMediaCacheManager) where LocationType : AlexMediaCacheLocation {
        delegate?.inCaching(progress: progress)
        print("缓存进度：\(progress)")
        controlPanel.cacheView.frame = CGRect(x: 48, y: 20, width: (Alex_SCREEN_WIDTH - 176) * CGFloat(progress), height: 1)
    }
    
    /** 缓存开始*/
    func cacher<LocationType>(_ cacher: AlexMediaCacher<LocationType>, didStartCacheOf cache: AlexMediaCacheManager) where LocationType : AlexMediaCacheLocation {
        delegate?.startCache()
    }
    
    /** 缓存完成*/
    func cacher<LocationType>(_ cacher: AlexMediaCacher<LocationType>, didFinishCacheOf cache: AlexMediaCacheManager) where LocationType : AlexMediaCacheLocation {
        delegate?.completeCache()
    }
    
    /** 缓存失败*/
    func cacher<LocationType>(_ cacher: AlexMediaCacher<LocationType>, didFailToCache cache: AlexMediaCacheManager) where LocationType : AlexMediaCacheLocation {
        delegate?.faildCache()
    }
    
}

extension AlexPlayerCommonView: AVPictureInPictureControllerDelegate {
    /**
