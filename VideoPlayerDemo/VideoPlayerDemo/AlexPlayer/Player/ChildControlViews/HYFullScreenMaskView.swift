...
protocol AlexFullScreenMaskViewDelegate: NSObjectProtocol {
...
class AlexFullScreenMaskView: UIView {
    
    weak var delegate: AlexFullScreenMaskViewDelegate?
    
    /// 全屏锁定
    var isScreenLock = false {
        didSet {
            lockBtn.setImage(UIImage(named: isScreenLock ? "alex_fullscreen_lock" : "alex_fullscreen_unlock", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        }
    }
    ...
    /// 退出全屏
    var backBtn: AlexResponseExpandButtton = {
        let btn = AlexResponseExpandButtton.init(type: .custom)
        btn.setImage(UIImage(named: "alex_video_ic_back", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    ...
    /// 更多功能
    var moreBtn: AlexResponseExpandButtton = {
        let btn = AlexResponseExpandButtton()
        btn.setImage(UIImage(named: "alex_fullscreen_ic_more", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    
    /// 屏幕锁定按钮
    var lockBtn: AlexResponseExpandButtton = {
        let btn = AlexResponseExpandButtton()
        btn.setImage(UIImage(named: "alex_fullscreen_unlock", in: Alex_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    ...
    private var speedCollectionView: UICollectionView = { () -> UICollectionView in
        ...
        collectionView.register(AlexMoreFunctionSpeedCollectionViewCell.self, forCellWithReuseIdentifier: "AlexMoreFunctionSpeedCollectionViewCell")
    ...
    override init(frame: CGRect) {
        ...
        // 读区本地存储播放速率
        if let rate = UserDefaults.standard.value(forKey: "AlexPlayer_rate") as? Float {
            currentSpeed = rate
        }
    ...
    private func createMoreFunctionView() {
        moreFunctionView.frame = CGRect(x: Alex_SCREEN_HEIGHT, y: 64, width: 100, height: Alex_SCREEN_WIDTH - (Alex_IS_IPHONEX ? 119 : 104))
    ...
    func showMoreFunctionView(isVerticalScreen: Bool) {
        ...
        UIView.animate(withDuration: 0.2) {
            if isVerticalScreen {
                self.moreFunctionView.frame = CGRect(x: Alex_SCREEN_WIDTH - self.moreFunctionWidth, y: 64, width: self.moreFunctionWidth, height: Alex_SCREEN_HEIGHT - (Alex_IS_IPHONEX ? 119 : 104))
            } else {
                self.moreFunctionView.frame = CGRect(x: Alex_SCREEN_HEIGHT - self.moreFunctionWidth, y: 64, width: self.moreFunctionWidth, height: Alex_SCREEN_WIDTH - (Alex_IS_IPHONEX ? 119 : 104))
            }
        }
    }
    ...
    func hidMoreFunctionView() {
        ...
        UIView.animate(withDuration: 0.2) {
            self.moreFunctionView.frame = CGRect(x: Alex_SCREEN_HEIGHT, y: 64, width: self.moreFunctionWidth, height: Alex_SCREEN_WIDTH - (Alex_IS_IPHONEX ? 119 : 104))
        }
    }
    ...
    @objc private func changePlayerSpeed(_ btn: UIButton) {
        if speedArray.count > btn.tag {
            if currentSpeed != speedArray[btn.tag] {
                ...
                // 存储改变速率到本地
                UserDefaults.standard.setValue(speedArray[btn.tag], forKey: "AlexPlayer_rate")
                // 给播放器返回当前速率
...
extension AlexFullScreenMaskView: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return speedArray.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "AlexMoreFunctionSpeedCollectionViewCell", for: indexPath) as! AlexMoreFunctionSpeedCollectionViewCell
