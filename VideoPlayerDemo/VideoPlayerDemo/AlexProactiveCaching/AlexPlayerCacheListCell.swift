...
class AlexPlayerCacheListCell: UITableViewCell {
    ...
    var size: Float?
    /// 缓存进度指示器
    var cacheProgressView: AlexCacheProgressView?
    ...
    /// 删除按钮
    var deleteButton: AlexPlayerCacheListCellButton = {
        let btn = AlexPlayerCacheListCellButton()
    ...
    private func createUI() {
        ...
        cacheProgressView = AlexCacheProgressView(frame: CGRect(x: Alex_SCREEN_WIDTH - 36, y: 10, width: 24, height: 24))
        cacheProgressView?.isHidden = true
        contentView.addSubview(cacheProgressView!)
    }
}

class AlexPlayerCacheListCellButton: UIButton {
    var mp4URL: URL?
}
