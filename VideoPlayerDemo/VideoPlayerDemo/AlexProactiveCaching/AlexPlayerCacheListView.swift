...
protocol AlexPlayerCacheListViewDelegate: NSObjectProtocol {
...
class AlexPlayerCacheListView: UIView {
    
    weak var delegate: AlexPlayerCacheListViewDelegate?
    ...
    var tableView: UITableView = { () -> UITableView in
        ...
        tableView.register(AlexPlayerCacheListCell.self, forCellReuseIdentifier: "AlexPlayerCacheListCell")
    ...
    private var cacheList: [AlexPlayerCommonConfig] = []
    
    private var expandingSectionSet: Set<Int> = [0]
    
    private var selectedIndexPaths: [IndexPath] = []
    
    private var videoCacher: AlexMediaCacher<AlexDefaultVideoCacheLocation>?
    ...
    private func createUI() {
        ...
        bottomView.snp.makeConstraints { (make) in
            make.leading.trailing.bottom.equalToSuperview()
            make.height.equalTo(Alex_IS_IPHONEX ? 77 : 48)
        }
    ...
    @objc private func downloadBtnPressed() {
        ...
        if
            let free = AlexStorage.available,
            ...
            if self?.cacheList.count ?? 0 > indexPath.row {
                if let cacheConfig = self?.cacheList[indexPath.row] {
                    if let urlString = cacheConfig.videoUrl {
                        ...
                        // 设置缓存标志
                        if let cell = tableView.cellForRow(at: indexPath) as? AlexPlayerCacheListCell, let url = URL(string: urlString) {
        ...
        if AlexVideoCacheManager.shared.cacheVideos(with: mp4URLs) {
            // 开始缓存
    ...
    /** 删除按钮被点击*/
    @objc private func deleteButtonDidClicked(_ sender: AlexPlayerCacheListCellButton) {
...
extension AlexPlayerCacheListView {
    
    /// 配置缓存列表
    /// - Parameters:
    ///   - cacheList: 视频配置集合
    ///   - videoCacher: 缓存对象
    func configView(cacheList: [AlexPlayerCommonConfig], videoCacher: AlexMediaCacher<AlexDefaultVideoCacheLocation>) {
    ...
    func reloadCache(videoCacher: AlexMediaCacher<AlexDefaultVideoCacheLocation>) {
        self.videoCacher = videoCacher
        cacheLab.text = ""
        tableView.reloadData()
    }
    ...
    func updateCacheProgress(cacheIdentifier: String, progress: CGFloat) {
        for indexPath in selectedIndexPaths {
            if let cell = tableView.cellForRow(at: indexPath) as? AlexPlayerCacheListCell {
...
extension AlexPlayerCacheListView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cacheList.count > indexPath.row {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "AlexPlayerCacheListCell", for: indexPath) as! AlexPlayerCacheListCell
                ...
                let url = URL(string: mp4URL)
            {
                ...
                cell.playUrl = mp4URL
                //TODO:
                let location = AlexDefaultVideoCacheLocation(remoteURL: url, mediaType: isVideo ? .video : .audio)
    ...
    func tableView(_ tableView: UITableView, willSelectRowAt indexPath: IndexPath) -> IndexPath? {
        ...
        let cell = tableView.cellForRow(at: indexPath) as? AlexPlayerCacheListCell
